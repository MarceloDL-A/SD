...
async function stageHistoryFields() {
  var cc = DataStudioApp.createCommunityConnector();
  var fields = cc.getFields();
  var types = cc.FieldType;
  ...
  var originName = fields
    ...
    .setGroup("Deals History")
    .setType(types.TEXT);
    var fieldsSetDefault;
    fieldsSetDefault = fields.setDefaultDimension(id.getId());
    cc = await cc
    fields = await fields
    types = await types
    id = await id
    ...
    dealId = await dealId
    inStageId = await inStageId
    outStageId = await outStageId
    inUserId = await inUserId
    outUserId = await outUserId
    inDate = await inDate
    outDate = await outDate
    inStageName = await inStageName
    outStageName = await outStageName
    inPipelineName = await inPipelineName
    leadTime = await leadTime
    inUserName = await inUserName
    outUserName = await outUserName
    createdAt = await createdAt
    uodatedAt = await uodatedAt
    freezed = await freezed
    status = await status
    closedAt = await closedAt
    valueMRR = await valueMRR
    value = await value
    originName = await originName
    fieldsSetDefault = await fieldsSetDefault

...
/**
 * Mandatory async function required by Google Data Studio that should
 * return the schema for requested fields and the formatted rows with data following
 * the schema.
 * This async function is called everytime a change is made in a report
 * @param {Object} request
 * @return {Object} data
 */
...
token = await token
table = await table
dateWindow = await dateWindow
startDate = await startDate
aggregations = await aggregations
client = await client
...
// async function prepareSchema(request) {
